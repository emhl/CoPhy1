...
double k2(double E, double x) { return x - 2 * E; }
...
double numerov(double E, int save) {
  ...
  // rechte seite
  for (x = H; x <= XMAX; x += H) {
    fnp1 = (H * H / 12.0 * (F(x + H) + 10.0 * F(x) + F(x - H)) +
            fn * (2.0 - 5.0 / 6.0 * H * H * k2(E, x)) -
            fnm1 * (1.0 + H * H / 12.0 * k2(E, x - H))) /
           (1.0 + H * H / 12.0 * k2(E, x + H));
    if (save == 1) {
      printf("%g %g\n", x, fn);
    }
